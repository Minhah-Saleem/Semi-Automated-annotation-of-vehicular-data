...
def writeOutput():
	...
	filename_2 = f_name.split('.')
	fh.save_annotation(drivename, fname, frame["file"])

	json_data = frame["file"]
	os.system("rd {}/*".format(os.path.join(DIR_PATH, "static/images")))
	#get_mask_rcnn_labels(filename_2)
	predicted_label = predict_label(json_data, f_name)
	#print(predicted_label)
	in_fov = os.path.exists(os.path.join(DIR_PATH, "static/images/cropped_image.png"))
	return str("hi")
